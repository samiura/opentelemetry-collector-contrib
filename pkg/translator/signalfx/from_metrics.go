...
func (ft *FromTranslator) FromMetrics(md pmetric.Metrics, dropHistogram bool) ([]*sfxpb.DataPoint, error) {
	...
	for i := 0; i < rms.Len(); i++ {
		...
		for j := 0; j < rm.ScopeMetrics().Len(); j++ {
			...
			for k := 0; k < ilm.Metrics().Len(); k++ {
				sfxDataPoints = append(sfxDataPoints, ft.FromMetric(ilm.Metrics().At(k), extraDimensions, dropHistogram)...)
			}
		}
	}
...
func (ft *FromTranslator) FromMetric(m pmetric.Metric, extraDimensions []*sfxpb.Dimension, dropHistogram bool) []*sfxpb.DataPoint {
	...
	case pmetric.MetricTypeHistogram:
		dps = convertHistogram(m.Histogram().DataPoints(), m.Name(), mt, extraDimensions, dropHistogram)
...
func convertHistogram(in pmetric.HistogramDataPointSlice, name string, mt *sfxpb.MetricType, extraDims []*sfxpb.Dimension, dropHistogram bool) []*sfxpb.DataPoint {
	var numDPs int
	for i := 0; i < in.Len(); i++ {
		histDP := in.At(i)
