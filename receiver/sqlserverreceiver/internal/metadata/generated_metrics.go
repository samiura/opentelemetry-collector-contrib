...
const (
	...
	AttributeReplicaDirectionSent
	AttributeReplicaDirectionReceived
)

// String returns the string representation of the AttributeReplicaDirection.
func (av AttributeReplicaDirection) String() string {
	switch av {
	case AttributeReplicaDirectionSent:
		return "sent"
	case AttributeReplicaDirectionReceived:
		return "received"
	}
	return ""
}
...
var MapAttributeReplicaDirection = map[string]AttributeReplicaDirection{
	"sent":     AttributeReplicaDirectionSent,
	"received": AttributeReplicaDirectionReceived,
}
...
type metricSqlserverPageFreeListStallsRate struct {
	data     pmetric.Metric // data buffer for generated metric.
	config   MetricConfig   // metric config provided by user.
	capacity int            // max observed number of data points added to the metric.
}

// init fills sqlserver.page.free_list_stalls.rate metric with initial data.
func (m *metricSqlserverPageFreeListStallsRate) init() {
	m.data.SetName("sqlserver.page.free_list_stalls.rate")
	m.data.SetDescription("Number of free list stalls.")
	m.data.SetUnit("“{stalls}/s”")
	m.data.SetEmptyGauge()
}

func (m *metricSqlserverPageFreeListStallsRate) recordDataPoint(start pcommon.Timestamp, ts pcommon.Timestamp, val int64) {
	if !m.config.Enabled {
		return
	}
	dp := m.data.Gauge().DataPoints().AppendEmpty()
...
// updateCapacity saves max length of data point slices that will be used for the slice capacity.
func (m *metricSqlserverPageFreeListStallsRate) updateCapacity() {
	if m.data.Gauge().DataPoints().Len() > m.capacity {
		m.capacity = m.data.Gauge().DataPoints().Len()
	}
}
...
// emit appends recorded metric data to a metrics slice and prepares it for recording another set of data points.
func (m *metricSqlserverPageFreeListStallsRate) emit(metrics pmetric.MetricSlice) {
	if m.config.Enabled && m.data.Gauge().DataPoints().Len() > 0 {
		m.updateCapacity()
		m.data.MoveTo(metrics.AppendEmpty())
		m.init()
...
func newMetricSqlserverPageFreeListStallsRate(cfg MetricConfig) metricSqlserverPageFreeListStallsRate {
	m := metricSqlserverPageFreeListStallsRate{config: cfg}
	if cfg.Enabled {
		m.data = pmetric.NewMetric()
		m.init()
	}
...
func (m *metricSqlserverReplicaDataRate) init() {
	...
	m.data.SetDescription("Total number of replica bytes.")
	m.data.SetUnit("“{by}/s”")
	m.data.SetEmptyGauge()
	m.data.Gauge().DataPoints().EnsureCapacity(m.capacity)
}
...
type MetricsBuilder struct {
	...
	metricSqlserverPageFreeListStallsRate             metricSqlserverPageFreeListStallsRate
...
func NewMetricsBuilder(mbc MetricsBuilderConfig, settings receiver.Settings, options ...MetricBuilderOption) *MetricsBuilder {
	...
	mb := &MetricsBuilder{
		...
		metricSqlserverPageFreeListStallsRate:             newMetricSqlserverPageFreeListStallsRate(mbc.Metrics.SqlserverPageFreeListStallsRate),
...
func (mb *MetricsBuilder) EmitForResource(options ...ResourceMetricsOption) {
	...
	mb.metricSqlserverPageFreeListStallsRate.emit(ils.Metrics())
...
// RecordSqlserverPageFreeListStallsRateDataPoint adds a data point to sqlserver.page.free_list_stalls.rate metric.
func (mb *MetricsBuilder) RecordSqlserverPageFreeListStallsRateDataPoint(ts pcommon.Timestamp, val int64) {
	mb.metricSqlserverPageFreeListStallsRate.recordDataPoint(mb.startTime, ts, val)
}
