...
func TestMetricsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allMetricsCount++
			mb.RecordSqlserverPageFreeListStallsRateDataPoint(ts, 1)
			...
			allMetricsCount++
			mb.RecordSqlserverReplicaDataRateDataPoint(ts, 1, AttributeReplicaDirectionSent)
			...
			for i := 0; i < ms.Len(); i++ {
				...
				case "sqlserver.page.checkpoint.flush.rate":
					...
					assert.Equal(t, pmetric.NumberDataPointValueTypeDouble, dp.ValueType())
					assert.InDelta(t, float64(1), dp.DoubleValue(), 0.01)
				case "sqlserver.page.free_list_stalls.rate":
					assert.False(t, validatedMetrics["sqlserver.page.free_list_stalls.rate"], "Found a duplicate in the metrics slice: sqlserver.page.free_list_stalls.rate")
					validatedMetrics["sqlserver.page.free_list_stalls.rate"] = true
					assert.Equal(t, pmetric.MetricTypeGauge, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Gauge().DataPoints().Len())
				...
				case "sqlserver.replica.data.rate":
					...
					assert.Equal(t, pmetric.MetricTypeGauge, ms.At(i).Type())
					assert.Equal(t, 1, ms.At(i).Gauge().DataPoints().Len())
					assert.Equal(t, "Total number of replica bytes.", ms.At(i).Description())
